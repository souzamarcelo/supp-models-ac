...
#include <grammar_parameters.cpp>
...
void tabu_search() {
  ...
  switch(option) {
    ...
    case 0:
      preamble += "BTS bt;\n";
      body += "tabusearch(S, bt, [&]() { return " + determineTenure() + "; }, start_time, target, maxstagnate, maxsteps);\n";
      break;
...
void step_generator() {
  ...
  switch(option) {
    case 0: {
      preamble += "StepGenerator sg" + to_string(level) + "(" + to_string(pD1) + ", I.n>=" + to_string(pD1) + "*" + to_string(pD2) + "?I.n/" + to_string(pD2) + ":" + to_string(pD1) + ");\n";
      body += "sg" + to_string(level);
      break;
    }

    case 1: {
      preamble += "StepGaussian sg" + to_string(level) + "(" + to_string(pD1) + ", I.n>=" + to_string(pD1) + "*" + to_string(pD2) + "?I.n/" + to_string(pD2) + ":" + to_string(pD1) + ", I.n);\n";
      body += "sg" + to_string(level);
      break;
    }

    case 2: {
      preamble += "StepExponential sg" + to_string(level) + "(" + to_string(pD1) + ", I.n>=" + to_string(pD1) + "*" + to_string(pD2) + "?I.n/" + to_string(pD2) + ":" + to_string(pD1) + ", I.n);\n";
      body += "sg" + to_string(level);
      break;
    }
...
void iterated_local_search_elite() {
  ...
  body += " },\n";
  perturbation(true);
  body += "e" + to_string(level) + ", start_time, target);\n";
...
void grammar() {
  preamble += "void run(Solution& S, po::variables_map& vm, chrono::system_clock::time_point start_time) {\n";
  algorithm();
  body += "}\n";
