...
unsigned bi(Solution& S) {
  /*
  double best = 0;
  int v1 = -1;
  int v2 = -1;
  
  for(int d = 0; d < S.I.desks; d++) {
    unsigned start = S.I.colors * d;
    int color = -1;
    for(int i = start; i < start + S.I.colors; i++) {
      if(S.x[i]) {
        color = i;
        break;
      }
    ...
    for(int i = start; i < start + S.I.colors; i++) {
      if(i != color) {
        double value = S.swapvalue(i, color);
        if(value < best) {
          best = value;
          v1 = i;
          v2 = color;
...
struct RII {
  ...
  unsigned blmr(Solution& S, Improvement improve) {
    /*
    if (random01(rng) < p) {
      randomMove(S);
    } else {
  ...
  void randomMove(Solution& S) {
    int desk = randomInt(rng) % S.I.desks;
    int start = desk * S.I.colors;
    
    if(!S.x[start]) {
      int color = -1;
      for(int i = start + 1; i < start + S.I.colors; i++) {
        if(S.x[i]) {
          color = i;
          break;
        }
      ...
      int index = color;
      while(index == color || index == 0)
        index = randomInt(rng) % S.I.colors;
      index += start;
      ...
      int desk2 = randomInt(rng) % S.I.desks;
      int start2 = desk2 * S.I.colors;
      while(S.x[start2]) {
        desk2 = randomInt(rng) % S.I.desks;
        start2 = desk2 * S.I.colors;
      }

      S.flip(start);
      int index = randomInt(rng) % S.I.colors;
      while(index == 0) {
        index = randomInt(rng) % S.I.colors;
      }
      S.flip(start + index);
      ...
      S.flip(start2);
      for(int i = start2 + 1; i < start2 + S.I.colors; i++) {
        if(S.x[i])
          S.flip(i);
      }
    }
...
unsigned localsearch(Solution& S, Improvement improve, chrono::system_clock::time_point start, int target) {
  ...
  do {
    ...
    if (S.value < B.value) {
      ...
      //cout << "Found better solution at " << chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - start).count() << " seconds." << endl;
      //if (record)
