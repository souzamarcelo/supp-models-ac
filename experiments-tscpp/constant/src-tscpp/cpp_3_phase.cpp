...
void tabuExploreSearch(){
	...
	while (non_improve_cnt < nnode ){
		...
		if (pcnt[oldpart] == 1)
			...
			tabutbl[curvertex][EMPTY_IDX] = tabu_itr + tenure;
		...
		//	tabutbl[curvertex][EMPTY_IDX] = tabutenure_base + rand() % pbkt_size;
			...
			tabutbl[curvertex][oldpart] = tabu_itr + tenure;
	...
	tabu_itr = tabu_itr + tenure; //jump D step to invalid the data in tabu table
...
//	int param_max_queue_size =  10 + random() % 10;
	...
	int param_max_queue_size = candpert;
...
void readParameters(int argc, char **argv){
	...
	for (int i = 1; i < argc; i+=2){
		...
		//	param_coftabu = atoi(argv[i+1]);
			...
			tenure = atoi(argv[i+1]);
		}else if (argv[i][1] == 'c'){
			candpert = atoi(argv[i+1]);
		}else if (argv[i][1] == 'p'){
			pertsize = atoi(argv[i+1]);
		}
	}
	/*check parameters*/
